...
Flashes the physical NTAG215 spool tag parked at Slot 1 with all supported tag formats:
...
For each format:
  - Writes pages 4-39 using direct RC522 passthrough via Moonraker HTTP
  - Verifies written bytes via RF read-back (100% byte-for-byte check)
  - Clears cached slot record on MCU (op 8)
  - Triggers live MCU FILAMENT_IDENTIFY (cmd 68)
  - Captures raw MCU protobuf reply directly from gcode_store
...
from ace_reader import Ace, PCD_TRANSCEIVE, BitFramingReg
...
def read_all_pages(a, first=4, last=39):
    """Read all pages in 4-page chunks (0x30 returns 16 bytes = 4 pages)."""
    out = {}
    page = first
    while page <= last:
        a.batch([(6, 0)])
...
def write_pages_diff(a, target_pages, rx0):
    current = read_all_pages(a, 4, 39)
    diff_pages = [p for p in sorted(target_pages.keys()) if current.get(p) != target_pages[p]]
    print(f"  Flashing {len(diff_pages)} changed pages (skipping {len(target_pages) - len(diff_pages)} identical)...")
    for p in diff_pages:
        write_page_raw(a, p, target_pages[p], rx0)
...
def clear_and_identify(a, slot=SLOT):
    """Clear cached record on MCU and trigger live hardware FILAMENT_IDENTIFY."""
    # Op 8: Clear cached tag record in MCU SRAM
    idx_clear = 0x80000000 | (READER << 24) | (8 << 16) | ((slot & 0x3F) << 8)
    _post(f"ACE_RAW_CMD T=0 CMD=FILAMENT_IDENTIFY INDEX={idx_clear}")
    time.sleep(0.15)

    # Issue live FILAMENT_IDENTIFY (cmd 68)
    _post(f"ACE_RAW_CMD T={slot} CMD=FILAMENT_IDENTIFY INDEX={slot}")
    time.sleep(0.6)

    # Fetch from gcode_store
    gs = _get("/server/gcode_store?count=30")["gcode_store"]
...
def run_all_tests():
    ...
    print("=" * 75)
    print(f"Target: Slot {SLOT}, Reader {READER} (Physical NTAG215 spool parked over antenna)")

    a = Ace(reader=READER, slot=SLOT)
    rx0 = prepare(a)
    print(f"Reader initialized: RxMode=0x{rx0:02X}")

    backup_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "slot1_original_backup.json")
    ...
    test_cases = [
        ...
        },
        {
            ...
            "pages": make_ndef_json_payload(
                '{"protocol":"openspool","version":"1.0","type":"PETG","color_hex":"#00FF00","brand":"Polymaker","temp_min":230,"temp_max":250,"bed_min":70,"bed_max":85,"sku":"SM50"}'
            ),
            "expected": {
                "version": 257,  # 0x0101
                "sku": "SM50",
        ...
        },
        {
            ...
            "pages": make_ndef_json_payload(
                '{"filaman":1,"sm_id":"88","brand":"FilaMan","material":"ABS","color_hex":"#0088FF","temp_min":240,"temp_max":260,"bed_min":90,"bed_max":110}'
            ),
            "expected": {
                "version": 257,  # 0x0101
                "sku": "SM88",
    ...
    try:
        for tc in test_cases:
            ...
            mcu_res = clear_and_identify(a, SLOT)
        ...
        print("=" * 75)
        try:
            write_pages_diff(a, original_pages, rx0)
            read_pages = read_all_pages(a, 4, 39)
        ...
        print("Restoring spool physical position (250mm feed)...")
        try:
            _post(f"ACE_RAW_FEED T={SLOT} MODE=0 LENGTH=250 SPEED=15")
            time.sleep(1.0)
